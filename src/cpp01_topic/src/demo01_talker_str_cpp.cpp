#include "rclcpp/rclcpp.hpp"
#include "std_msgs/msg/string.hpp"

using namespace std::chrono_literals;//可以后缀上一个时间单位

//创建一个节点类
class Talker: public rclcpp::Node{
public:
        Talker():Node("talker_node_cpp"),count(0){
            RCLCPP_INFO(this->get_logger(),"发布节点创建！");
            //创建一个向“chatter”话题发布string类型消息的publisher。
            publisher_=this->create_publisher<std_msgs::msg::String>("chatter",10);
            //timer_定义发送周期，绑定回调函数
            timer_=this->create_wall_timer(0.1s,std::bind(&Talker::on_timer,this));
        }

private:
void on_timer(){
        auto message=std_msgs::msg::String();
        message.data = "hello world!"+std::to_string(count++);
        RCLCPP_INFO(this->get_logger(),"发布方发布的消息：%s",message.data.c_str());
        publisher_->publish(message);
}
   rclcpp::Publisher<std_msgs::msg::String>::SharedPtr publisher_;
   rclcpp::TimerBase::SharedPtr timer_;
   size_t count;
};

int main(int argc, char ** argv)
{
    rclcpp::init(argc,argv);
    auto a=std::make_shared<Talker>();//用智能指针创建对象
    rclcpp::spin(a);//运行节点功能的一种常用的方式
    rclcpp::shutdown;


}